...
COMPOUNDQUERYFILTEROPERANDSINNER_ANY_OF_SCHEMAS = ["CompoundQueryFilter", "SimpleQueryFilter"]

class CompoundQueryFilterOperandsInner(BaseModel):
    """
    CompoundQueryFilterOperandsInner
    """

    # data type: SimpleQueryFilter
    anyof_schema_1_validator: Optional[SimpleQueryFilter] = None
    # data type: CompoundQueryFilter
    anyof_schema_2_validator: Optional[CompoundQueryFilter] = None
    if TYPE_CHECKING:
        actual_instance: Union[CompoundQueryFilter, SimpleQueryFilter]
    else:
        actual_instance: Any
    any_of_schemas: List[str] = Field(COMPOUNDQUERYFILTEROPERANDSINNER_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CompoundQueryFilterOperandsInner.construct()
        error_messages = []
        # validate data type: SimpleQueryFilter
        if not isinstance(v, SimpleQueryFilter):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SimpleQueryFilter`")
        else:
            return v

        # validate data type: CompoundQueryFilter
        if not isinstance(v, CompoundQueryFilter):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CompoundQueryFilter`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in CompoundQueryFilterOperandsInner with anyOf schemas: CompoundQueryFilter, SimpleQueryFilter. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> CompoundQueryFilterOperandsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> CompoundQueryFilterOperandsInner:
        """Returns the object represented by the json string"""
        instance = CompoundQueryFilterOperandsInner.construct()
        error_messages = []
        # anyof_schema_1_validator: Optional[SimpleQueryFilter] = None
        try:
            instance.actual_instance = SimpleQueryFilter.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[CompoundQueryFilter] = None
        try:
            instance.actual_instance = CompoundQueryFilter.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CompoundQueryFilterOperandsInner with anyOf schemas: CompoundQueryFilter, SimpleQueryFilter. Details: " + ", ".join(error_messages))
        else:
            return instance
...
CompoundQueryFilterOperandsInner.update_forward_refs()
