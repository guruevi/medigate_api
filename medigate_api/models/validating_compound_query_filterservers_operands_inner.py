...
VALIDATINGCOMPOUNDQUERYFILTERSERVERSOPERANDSINNER_ANY_OF_SCHEMAS = ["ValidatingCompoundQueryFilterservers", "ValidatingSimpleQueryFilterservers"]

class ValidatingCompoundQueryFilterserversOperandsInner(BaseModel):
    """
    ValidatingCompoundQueryFilterserversOperandsInner
    """

    # data type: ValidatingSimpleQueryFilterservers
    anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterservers] = None
    # data type: ValidatingCompoundQueryFilterservers
    anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterservers] = None
    if TYPE_CHECKING:
        actual_instance: Union[ValidatingCompoundQueryFilterservers, ValidatingSimpleQueryFilterservers]
    else:
        actual_instance: Any
    any_of_schemas: List[str] = Field(VALIDATINGCOMPOUNDQUERYFILTERSERVERSOPERANDSINNER_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ValidatingCompoundQueryFilterserversOperandsInner.construct()
        error_messages = []
        # validate data type: ValidatingSimpleQueryFilterservers
        if not isinstance(v, ValidatingSimpleQueryFilterservers):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingSimpleQueryFilterservers`")
        else:
            return v

        # validate data type: ValidatingCompoundQueryFilterservers
        if not isinstance(v, ValidatingCompoundQueryFilterservers):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingCompoundQueryFilterservers`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ValidatingCompoundQueryFilterserversOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterservers, ValidatingSimpleQueryFilterservers. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ValidatingCompoundQueryFilterserversOperandsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ValidatingCompoundQueryFilterserversOperandsInner:
        """Returns the object represented by the json string"""
        instance = ValidatingCompoundQueryFilterserversOperandsInner.construct()
        error_messages = []
        # anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterservers] = None
        try:
            instance.actual_instance = ValidatingSimpleQueryFilterservers.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterservers] = None
        try:
            instance.actual_instance = ValidatingCompoundQueryFilterservers.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ValidatingCompoundQueryFilterserversOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterservers, ValidatingSimpleQueryFilterservers. Details: " + ", ".join(error_messages))
        else:
            return instance
...
ValidatingCompoundQueryFilterserversOperandsInner.update_forward_refs()
