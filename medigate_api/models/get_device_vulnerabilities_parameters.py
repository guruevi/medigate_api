...
class GetDeviceVulnerabilitiesParameters(BaseModel):
    """
    GetDeviceVulnerabilitiesParameters
    """
    filter_by: Optional[FilterBy4] = None
    fields: conlist(DeviceVulnerabilityFieldsEnum, min_items=1) = Field(..., description="Specify which fields to return for each item. See the endpoint description for details of supported fields.")
    sort_by: Optional[conlist(ValidatingSortClauseDeviceVulnerability, min_items=1)] = Field(None, description="Specifies how the returned data should be sorted. If more than one sort clause is passed, additional clauses will be used to sort data that is equal in all previous clauses.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetDeviceVulnerabilitiesParameters:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetDeviceVulnerabilitiesParameters:
        """Create an instance of GetDeviceVulnerabilitiesParameters from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetDeviceVulnerabilitiesParameters.parse_obj(obj)

        _obj = GetDeviceVulnerabilitiesParameters.parse_obj({
            "filter_by": FilterBy4.from_dict(obj.get("filter_by")) if obj.get("filter_by") is not None else None,
            "fields": obj.get("fields"),
            "sort_by": [ValidatingSortClauseDeviceVulnerability.from_dict(_item) for _item in obj.get("sort_by")] if obj.get("sort_by") is not None else None,
