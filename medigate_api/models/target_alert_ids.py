...
class TargetAlertIds(BaseModel):
    """
    TargetAlertIds
    """
    alert_ids: conlist(StrictInt) = Field(..., description="A list of Alert IDs, as indicated in the `id` field of an alert.")
    __properties = ["alert_ids"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TargetAlertIds:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TargetAlertIds:
        """Create an instance of TargetAlertIds from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TargetAlertIds.parse_obj(obj)

        _obj = TargetAlertIds.parse_obj({
