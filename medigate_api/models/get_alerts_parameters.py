...
class GetAlertsParameters(BaseModel):
    """
    GetAlertsParameters
    """
    filter_by: Optional[FilterBy2] = None
    fields: conlist(AlertFieldsFieldsEnum, min_items=1) = Field(..., description="Specify which fields to return for each item. See the endpoint description for details of supported fields.")
    sort_by: Optional[conlist(ValidatingSortClauseAlertFields, min_items=1)] = Field(None, description="Specifies how the returned data should be sorted. If more than one sort clause is passed, additional clauses will be used to sort data that is equal in all previous clauses.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetAlertsParameters:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetAlertsParameters:
        """Create an instance of GetAlertsParameters from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetAlertsParameters.parse_obj(obj)

        _obj = GetAlertsParameters.parse_obj({
            "filter_by": FilterBy2.from_dict(obj.get("filter_by")) if obj.get("filter_by") is not None else None,
            "fields": obj.get("fields"),
            "sort_by": [ValidatingSortClauseAlertFields.from_dict(_item) for _item in obj.get("sort_by")] if obj.get("sort_by") is not None else None,
