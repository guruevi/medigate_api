...
class PublicSetAlertResolutionParams(BaseModel):
    """
    PublicSetAlertResolutionParams
    """
    alerts: TargetAlertIds = Field(...)
    devices: Optional[DeviceFilterBy] = None
    status: PublicAlertStatus = Field(..., description="Set the device-alert status to resolve or unresolved.")
    __properties = ["alerts", "devices", "status"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PublicSetAlertResolutionParams:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PublicSetAlertResolutionParams:
        """Create an instance of PublicSetAlertResolutionParams from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PublicSetAlertResolutionParams.parse_obj(obj)

        _obj = PublicSetAlertResolutionParams.parse_obj({
            "alerts": TargetAlertIds.from_dict(obj.get("alerts")) if obj.get("alerts") is not None else None,
            "devices": DeviceFilterBy.from_dict(obj.get("devices")) if obj.get("devices") is not None else None,
