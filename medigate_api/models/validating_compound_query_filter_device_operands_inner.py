...
VALIDATINGCOMPOUNDQUERYFILTERDEVICEOPERANDSINNER_ANY_OF_SCHEMAS = ["ValidatingCompoundQueryFilterDevice", "ValidatingSimpleQueryFilterDevice"]

class ValidatingCompoundQueryFilterDeviceOperandsInner(BaseModel):
    """
    ValidatingCompoundQueryFilterDeviceOperandsInner
    """

    # data type: ValidatingSimpleQueryFilterDevice
    anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterDevice] = None
    # data type: ValidatingCompoundQueryFilterDevice
    anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterDevice] = None
    if TYPE_CHECKING:
        actual_instance: Union[ValidatingCompoundQueryFilterDevice, ValidatingSimpleQueryFilterDevice]
    else:
        actual_instance: Any
    any_of_schemas: List[str] = Field(VALIDATINGCOMPOUNDQUERYFILTERDEVICEOPERANDSINNER_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ValidatingCompoundQueryFilterDeviceOperandsInner.construct()
        error_messages = []
        # validate data type: ValidatingSimpleQueryFilterDevice
        if not isinstance(v, ValidatingSimpleQueryFilterDevice):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingSimpleQueryFilterDevice`")
        else:
            return v

        # validate data type: ValidatingCompoundQueryFilterDevice
        if not isinstance(v, ValidatingCompoundQueryFilterDevice):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingCompoundQueryFilterDevice`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ValidatingCompoundQueryFilterDeviceOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterDevice, ValidatingSimpleQueryFilterDevice. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ValidatingCompoundQueryFilterDeviceOperandsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ValidatingCompoundQueryFilterDeviceOperandsInner:
        """Returns the object represented by the json string"""
        instance = ValidatingCompoundQueryFilterDeviceOperandsInner.construct()
        error_messages = []
        # anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterDevice] = None
        try:
            instance.actual_instance = ValidatingSimpleQueryFilterDevice.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterDevice] = None
        try:
            instance.actual_instance = ValidatingCompoundQueryFilterDevice.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ValidatingCompoundQueryFilterDeviceOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterDevice, ValidatingSimpleQueryFilterDevice. Details: " + ", ".join(error_messages))
        else:
            return instance
...
ValidatingCompoundQueryFilterDeviceOperandsInner.update_forward_refs()
