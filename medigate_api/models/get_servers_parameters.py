...
class GetServersParameters(BaseModel):
    """
    GetServersParameters
    """
    filter_by: Optional[FilterBy8] = None
    fields: conlist(ServersFieldsEnum, min_items=1) = Field(..., description="Specify which fields to return for each item. See the endpoint description for details of supported fields.")
    sort_by: Optional[conlist(ValidatingSortClauseservers)] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetServersParameters:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetServersParameters:
        """Create an instance of GetServersParameters from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetServersParameters.parse_obj(obj)

        _obj = GetServersParameters.parse_obj({
            "filter_by": FilterBy8.from_dict(obj.get("filter_by")) if obj.get("filter_by") is not None else None,
            "fields": obj.get("fields"),
            "sort_by": [ValidatingSortClauseservers.from_dict(_item) for _item in obj.get("sort_by")] if obj.get("sort_by") is not None else None,
