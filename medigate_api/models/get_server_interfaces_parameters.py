...
class GetServerInterfacesParameters(BaseModel):
    """
    GetServerInterfacesParameters
    """
    filter_by: Optional[FilterBy7] = None
    fields: conlist(ServerInterfacesFieldsEnum, min_items=1) = Field(..., description="Specify which fields to return for each item. See the endpoint description for details of supported fields.")
    sort_by: Optional[conlist(ValidatingSortClauseserverInterfaces)] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetServerInterfacesParameters:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetServerInterfacesParameters:
        """Create an instance of GetServerInterfacesParameters from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetServerInterfacesParameters.parse_obj(obj)

        _obj = GetServerInterfacesParameters.parse_obj({
            "filter_by": FilterBy7.from_dict(obj.get("filter_by")) if obj.get("filter_by") is not None else None,
            "fields": obj.get("fields"),
            "sort_by": [ValidatingSortClauseserverInterfaces.from_dict(_item) for _item in obj.get("sort_by")] if obj.get("sort_by") is not None else None,
