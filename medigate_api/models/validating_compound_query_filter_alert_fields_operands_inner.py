...
VALIDATINGCOMPOUNDQUERYFILTERALERTFIELDSOPERANDSINNER_ANY_OF_SCHEMAS = ["ValidatingCompoundQueryFilterAlertFields", "ValidatingSimpleQueryFilterAlertFields"]

class ValidatingCompoundQueryFilterAlertFieldsOperandsInner(BaseModel):
    """
    ValidatingCompoundQueryFilterAlertFieldsOperandsInner
    """

    # data type: ValidatingSimpleQueryFilterAlertFields
    anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterAlertFields] = None
    # data type: ValidatingCompoundQueryFilterAlertFields
    anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterAlertFields] = None
    if TYPE_CHECKING:
        actual_instance: Union[ValidatingCompoundQueryFilterAlertFields, ValidatingSimpleQueryFilterAlertFields]
    else:
        actual_instance: Any
    any_of_schemas: List[str] = Field(VALIDATINGCOMPOUNDQUERYFILTERALERTFIELDSOPERANDSINNER_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ValidatingCompoundQueryFilterAlertFieldsOperandsInner.construct()
        error_messages = []
        # validate data type: ValidatingSimpleQueryFilterAlertFields
        if not isinstance(v, ValidatingSimpleQueryFilterAlertFields):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingSimpleQueryFilterAlertFields`")
        else:
            return v

        # validate data type: ValidatingCompoundQueryFilterAlertFields
        if not isinstance(v, ValidatingCompoundQueryFilterAlertFields):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingCompoundQueryFilterAlertFields`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ValidatingCompoundQueryFilterAlertFieldsOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterAlertFields, ValidatingSimpleQueryFilterAlertFields. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ValidatingCompoundQueryFilterAlertFieldsOperandsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ValidatingCompoundQueryFilterAlertFieldsOperandsInner:
        """Returns the object represented by the json string"""
        instance = ValidatingCompoundQueryFilterAlertFieldsOperandsInner.construct()
        error_messages = []
        # anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterAlertFields] = None
        try:
            instance.actual_instance = ValidatingSimpleQueryFilterAlertFields.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterAlertFields] = None
        try:
            instance.actual_instance = ValidatingCompoundQueryFilterAlertFields.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ValidatingCompoundQueryFilterAlertFieldsOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterAlertFields, ValidatingSimpleQueryFilterAlertFields. Details: " + ", ".join(error_messages))
        else:
            return instance
...
ValidatingCompoundQueryFilterAlertFieldsOperandsInner.update_forward_refs()
