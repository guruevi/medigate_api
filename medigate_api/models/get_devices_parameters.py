...
class GetDevicesParameters(BaseModel):
    """
    GetDevicesParameters
    """
    filter_by: Optional[FilterBy5] = None
    fields: conlist(DeviceFieldsEnum, min_items=1) = Field(..., description="Specify which fields to return for each item. See the endpoint description for details of supported fields.")
    sort_by: Optional[conlist(ValidatingSortClauseDevice, min_items=1)] = Field(None, description="Specifies how the returned data should be sorted. If more than one sort clause is passed, additional clauses will be used to sort data that is equal in all previous clauses.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetDevicesParameters:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetDevicesParameters:
        """Create an instance of GetDevicesParameters from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetDevicesParameters.parse_obj(obj)

        _obj = GetDevicesParameters.parse_obj({
            "filter_by": FilterBy5.from_dict(obj.get("filter_by")) if obj.get("filter_by") is not None else None,
            "fields": obj.get("fields"),
            "sort_by": [ValidatingSortClauseDevice.from_dict(_item) for _item in obj.get("sort_by")] if obj.get("sort_by") is not None else None,
