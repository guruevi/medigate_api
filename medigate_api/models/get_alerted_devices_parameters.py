...
class GetAlertedDevicesParameters(BaseModel):
    """
    GetAlertedDevicesParameters
    """
    filter_by: Optional[FilterBy1] = None
    fields: conlist(AlertedDeviceFieldsEnum, min_items=1) = Field(..., description="Specify which fields to return for each item. See the endpoint description for details of supported fields.")
    sort_by: Optional[conlist(ValidatingSortClauseAlertedDevice, min_items=1)] = Field(None, description="Specifies how the returned data should be sorted. If more than one sort clause is passed, additional clauses will be used to sort data that is equal in all previous clauses.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetAlertedDevicesParameters:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetAlertedDevicesParameters:
        """Create an instance of GetAlertedDevicesParameters from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetAlertedDevicesParameters.parse_obj(obj)

        _obj = GetAlertedDevicesParameters.parse_obj({
            "filter_by": FilterBy1.from_dict(obj.get("filter_by")) if obj.get("filter_by") is not None else None,
            "fields": obj.get("fields"),
            "sort_by": [ValidatingSortClauseAlertedDevice.from_dict(_item) for _item in obj.get("sort_by")] if obj.get("sort_by") is not None else None,
