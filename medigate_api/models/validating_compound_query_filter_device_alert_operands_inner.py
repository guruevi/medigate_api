...
VALIDATINGCOMPOUNDQUERYFILTERDEVICEALERTOPERANDSINNER_ANY_OF_SCHEMAS = ["ValidatingCompoundQueryFilterDeviceAlert", "ValidatingSimpleQueryFilterDeviceAlert"]

class ValidatingCompoundQueryFilterDeviceAlertOperandsInner(BaseModel):
    """
    ValidatingCompoundQueryFilterDeviceAlertOperandsInner
    """

    # data type: ValidatingSimpleQueryFilterDeviceAlert
    anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterDeviceAlert] = None
    # data type: ValidatingCompoundQueryFilterDeviceAlert
    anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterDeviceAlert] = None
    if TYPE_CHECKING:
        actual_instance: Union[ValidatingCompoundQueryFilterDeviceAlert, ValidatingSimpleQueryFilterDeviceAlert]
    else:
        actual_instance: Any
    any_of_schemas: List[str] = Field(VALIDATINGCOMPOUNDQUERYFILTERDEVICEALERTOPERANDSINNER_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ValidatingCompoundQueryFilterDeviceAlertOperandsInner.construct()
        error_messages = []
        # validate data type: ValidatingSimpleQueryFilterDeviceAlert
        if not isinstance(v, ValidatingSimpleQueryFilterDeviceAlert):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingSimpleQueryFilterDeviceAlert`")
        else:
            return v

        # validate data type: ValidatingCompoundQueryFilterDeviceAlert
        if not isinstance(v, ValidatingCompoundQueryFilterDeviceAlert):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ValidatingCompoundQueryFilterDeviceAlert`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ValidatingCompoundQueryFilterDeviceAlertOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterDeviceAlert, ValidatingSimpleQueryFilterDeviceAlert. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ValidatingCompoundQueryFilterDeviceAlertOperandsInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ValidatingCompoundQueryFilterDeviceAlertOperandsInner:
        """Returns the object represented by the json string"""
        instance = ValidatingCompoundQueryFilterDeviceAlertOperandsInner.construct()
        error_messages = []
        # anyof_schema_1_validator: Optional[ValidatingSimpleQueryFilterDeviceAlert] = None
        try:
            instance.actual_instance = ValidatingSimpleQueryFilterDeviceAlert.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ValidatingCompoundQueryFilterDeviceAlert] = None
        try:
            instance.actual_instance = ValidatingCompoundQueryFilterDeviceAlert.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ValidatingCompoundQueryFilterDeviceAlertOperandsInner with anyOf schemas: ValidatingCompoundQueryFilterDeviceAlert, ValidatingSimpleQueryFilterDeviceAlert. Details: " + ", ".join(error_messages))
        else:
            return instance
...
ValidatingCompoundQueryFilterDeviceAlertOperandsInner.update_forward_refs()
